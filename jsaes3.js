...
// The Sbox and Sbox_Inv tables are the inverses of one another eg. 2A => E5 in Sbox
// and E5 => 2A in Sbox_Inv
...
/* num2String: convert block to somewhat garbled output of ASCII characters */
function num2String(aDataBlock) {
  let arr = [];
  for( let v = 0; v < aDataBlock.length; v++) {
    arr.push(String.fromCharCode(aDataBlock[v]) );
  }
  return arr.join('');
}
...
function AES_ExpandKey(key) {
  ...
  console.log('keyHex: ' + keyHex + ' keyHex length: ' + keyHex.length + '\n');
  console.log('All Roundkeys \n');
  for(let j = 0, r = 0; j < key.length; j = j + 4, r++) {
    console.log( 'w['+ r + ']' + ' ' + (key[j]).toString(16) + ' ' + (key[j + 1]).toString(16) + ' ' + (key[j + 2]).toString(16) + ' ' + (key[j + 3]).toString(16) ) ;
     if(j % 4 == 0 )
      console.log('\n');
  }

 /* 
  for(let k = 0; k < keyHex.length; k = k + 4) {
    console.log( (keyHex[k]).toString(16) + ' ' + (keyHex[k + 1]).toString(16) + ' ' + (keyHex[k + 2]).toString(16) + ' ' + (keyHex[k + 3]).toString(16) ) ;
     if(k % 4 == 0 )
      console.log('\n');
  }
  */
...
function AES_Encrypt(block, key) {
  ...
  console.log('CipherText Decimal: ' + block);
  for(let r = 0; r < block.length; r++) {
    cipherText[r] = (block[r]).toString(16);
  }
  console.log('CipherText Hex: ' + cipherText);
  console.log('CipherText garble: ' + num2String(block) );
}
...
function AES_Decrypt(block, key) {
  ...
  for(let f = 0; f < block.length; f++) {
      blockDecipher[f] = (block[f]).toString(16);
  }
  console.log('Deciphered text Decimal: ' + block);
  console.log('Deciphered text Hex: ' + blockDecipher);
  console.log('Deciphered plaintext block : ' + num2String(block) );
/*
  blockDecipher2 = String.fromCharCode(block);

  console.log('blockDecipher2: ' + blockDecipher2);
*/
}
...
/* As noted earlier, to compute inverse, we */
